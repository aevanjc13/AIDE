...
#Region "Procedures"
    Public Sub Add()
        Try
            ...
            For Each rawEmployee As EmployeeModel In EmployeeList
                If SelectedEmployee.Name.Equals(rawEmployee.Name) Then
                    AssignedEmployeeList.Add(rawEmployee)
                End If
            Next

            For Each rawEmployee As EmployeeModel In EmployeeList
                If SelectedEmployee.Name.Equals(rawEmployee.Name) Then
                    EmployeeList.Remove(rawEmployee)
                End If
            Next
        Catch ex As Exception
        ...
        If AssignedEmployeeList.Count > 0 Then
            AddMode = True
            RemoveMode = True
            ClearMode = True
        End If
    ...
    Private Sub Clear()
        Try
            For Each rawEmployee As EmployeeModel In AssignedEmployeeList
                EmployeeList.Add(rawEmployee)
            Next
    ...
    Private Sub Remove()
        Try
            For Each rawEmployee As EmployeeModel In AssignedEmployeeList
                If SelectedAssignedEmployee.Name.Equals(rawEmployee.Name) Then
                    EmployeeList.Add(rawEmployee)
                End If
            Next
            ...
            For Each rawEmployee As EmployeeModel In AssignedEmployeeList
                If SelectedAssignedEmployee.Name.Equals(rawEmployee.Name) Then
                    AssignedEmployeeList.Remove(rawEmployee)
                End If
            Next
        Catch ex As Exception
            Console.WriteLine(ex.Message)
    ...
    Private Sub Create()
        ...
        Try
            ...
            For Each x In _assignedEmployeeList
                Dim objassign As New AssignedProject
                objassign.EmployeeID = x.EmployeeID
