...
namespace GDC.PH.AIDE.BusinessLayer.DataLayer
{
	...
	class clsEmployeeSql : DataLayerBase 
	{
        ...
        internal void PopulateBusinessObjectFromReader(clsEmployee businessObject, IDataReader dataReader)
        {
				...
				if (!dataReader.IsDBNull(dataReader.GetOrdinal(clsEmployee.clsEmployeeFields.MIDDLE_INITIAL.ToString())))
				{
					businessObject.MIDDLE_INITIAL = dataReader.GetString(dataReader.GetOrdinal(clsEmployee.clsEmployeeFields.MIDDLE_INITIAL.ToString()));
				}
