...
#Region "Action List"
    ...
    Public Overrides Function GetActionSummry(ByRef objResult As List(Of Action)) As Boolean
        Dim state As StateData = ActionMgmt.GetActionList()
        Dim bSuccess As Boolean = False
        If state.NotifyType = NotifyType.IsSuccess Then
            bSuccess = False
            objResult = state.Data
    ...
    Public Overrides Function GetActionLstByMessage(ByVal _Message As String, ByRef objResult As List(Of Action)) As Boolean
        Dim state As StateData = ActionMgmt.GetActionByMessage(_Message)
        Dim bSuccess As Boolean = False
        If state.NotifyType = NotifyType.IsSuccess Then
            bSuccess = False
            objResult = state.Data
...
#Region "Lessons Learnt"
    Public Overrides Function GetAllLessonLearntList(ByRef objResult As List(Of LessonLearnt)) As Boolean
        Dim state As StateData = LessonLearntMgmt.GetLessonLearntList()
        Dim bSuccess As Boolean = False
        If state.NotifyType = NotifyType.IsSuccess Then
            bSuccess = True
            objResult = state.Data
    ...
    Public Overrides Function GetLessonLearntByProblems(ByRef searchProblem As String, ByRef objResult As List(Of LessonLearnt)) As Boolean
        Dim state As StateData = LessonLearntMgmt.GetLessonLearntListByProblem(searchProblem)
        Dim bSuccess As Boolean = False
        If state.NotifyType = NotifyType.IsSuccess Then
            bSuccess = True
            objResult = state.Data
...
#Region "Concern Function"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Overrides Function GetListOfACtion(Ref_id As String) As List(Of Concern)
        Dim state As StateData = ConcernMgmt.GetListOfACtion(Ref_id)
        Dim lstConcernList As New List(Of Concern)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Overrides Function GetSearchAction(_keywordAction As String, Ref_id As String) As List(Of Concern)
        Dim state As StateData = ConcernMgmt.GetSearchAction(_keywordAction, Ref_id)
        Dim lstConcernList As New List(Of Concern)
...
#Region "Skills"
    ...
    Public Function ViewAllEmpSkills(deptID As Integer) As List(Of Skills)
        Dim state As StateData = SkillsMgmt.ViewAllEmpSKills(deptID)
        Dim skillsLst As New List(Of Skills)
...
#Region "Tasks"
    ...
    ' Get this Function
    Public Overrides Function ViewTasksSummaryAll(Current_Date As Date, ByRef objResult As List(Of TaskSummary)) As Boolean
        Dim state As StateData = TasksMgmt.ViewTaskAll(Current_Date)
        Dim bSuccess As Boolean = False
        If state.NotifyType = NotifyType.IsSuccess Then
            bSuccess = True
            objResult = state.Data
...
#Region "Resource Planner"
    ...
    Public Function ViewEmpResourcePlanners(deptID As Integer) As List(Of ResourcePlanner)
        Dim state As StateData = ResourceMgmt.ViewEmpResourcePlanner(deptID)
    ...
    Public Overrides Function GetAllEmpResourcePlanner(deptID As Integer, month As Integer, year As Integer) As List(Of ResourcePlanner)
        Dim state As StateData = ResourceMgmt.GetAllEmpResourcePlanner(deptID, month, year)
        Dim resourceLst As New List(Of ResourcePlanner)
    ...
    Public Overrides Function GetAllEmpResourcePlannerByStatus(deptID As Integer, month As Integer, year As Integer, status As Integer) As List(Of ResourcePlanner)
        Dim state As StateData = ResourceMgmt.GetAllEmpResourcePlannerByStatus(deptID, month, year, status)
        Dim resourceLst As New List(Of ResourcePlanner)
